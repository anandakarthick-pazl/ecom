            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <p class="mb-3">Amount to Pay: <strong
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="border-start border-primary border-3 ps-3">
                                    ...
                                    <p class="mb-1"><i class="fas fa-phone text-muted"></i> {{ $order->customer_mobile }}
                                    </p>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="order-items">
                                    ...
                                    @foreach ($order->items as $item)
                                        ...
                                        <div
                                            class="d-flex justify-content-between align-items-start mb-3 p-2 bg-light rounded">
                                            <div class="flex-grow-1">
                                                <div class="d-flex align-items-center">
                                                    <strong class="text-dark me-2">{{ $item->product_name }}</strong>
                                                    @if ($hasOffer && $discountPercentage > 0)
                                                        <span class="badge bg-danger">{{ round($discountPercentage) }}%
                                                            OFF</span>
                                                    @endif
                                                </div>
                                                ...
                                                <small class="text-muted d-block mt-1">
                                                    ...
                                                    @if ($hasOffer)
                                                        <span
                                                            class="text-success fw-bold">₹{{ number_format($effectivePrice, 2) }}</span>
                                                        <span
                                                            class="text-decoration-line-through ms-1">₹{{ number_format($originalPrice, 2) }}</span>
                                                    @else
                                                    ...
                                                    @if ($item->tax_percentage > 0)
                                                        <br><small class="text-muted"><i class="fas fa-info-circle"></i>
                                                            (Incl. GST {{ $item->tax_percentage }}%)</small>
                                                    @endif
                                                </small>

                                                @if ($hasOffer && $savings > 0)
                                                    <small class="text-success d-block mt-1">
                                                        <i class="fas fa-tags"></i> You saved
                                                        ₹{{ number_format($savings * $item->quantity, 2) }}
                                                        @if ($offerSource === 'offers_page')
                                            ...
                                            <div class="text-end">
                                                @if ($hasOffer)
                                                    <strong
                                                        class="text-success d-block">₹{{ number_format($effectivePrice * $item->quantity, 2) }}</strong>
                                                    <small class="text-decoration-line-through text-muted">
                                                        ₹{{ number_format($originalPrice * $item->quantity, 2) }}
                                                    </small>
                                                @else
                                                    <strong
                                                        class="text-success">₹{{ number_format($item->total, 2) }}</strong>
                                                @endif
                                                <br>
                                ...
                                @endif --}}
                                    ...
                                    @if ($order->discount > 0)
                                        <div class="d-flex justify-content-between mb-2">
                                            <span class="text-success"><i class="fas fa-percent"></i> Coupon
                                                Discount:</span>
                    ...
                    <div class="card-body text-center">
                        ...
                        <div class="d-flex justify-content-center gap-3">
                            @php
                                ...
                                $defaultFormat =
                                    \App\Models\AppSetting::getForTenant('default_bill_format', $companyId) ??
                                    'a4_sheet';
                                $thermalEnabled =
                                    \App\Models\AppSetting::getForTenant('thermal_printer_enabled', $companyId) ??
                                    false;
                                $a4Enabled =
                                    \App\Models\AppSetting::getForTenant('a4_sheet_enabled', $companyId) ?? true;
                            @endphp
                ...
                function initPaymentSystem() {
                    ...
                    if (payButton) {
                        payButton.addEventListener('click', function() {
                            ...
                            utils.updateButton('pay-now-btn', true,
                            ...
                            utils.makeRequest(
                                ...
                                function(response) {
                                    ...
                                    if (response.success) {
                                        console.log(
                                            '✅ Razorpay Estimate created successfully, opening payment gateway...'
                                            );
                                        ...
                                        const razorpayOptions = {
                                            ...
                                            modal: {
                                                ondismiss: function() {
                                                    ...
                                                    utils.updateButton('pay-now-btn', false,
                                                        '<i class="fas fa-credit-card"></i> Pay Now'
                                                        );
                                                    utils.showModal('razorpayModal');
                                                }
                                            ...
                                            handler: function(response) {
                                                console.log(
                                                    '💳 Payment completed successfully, starting verification...'
                                                    );
                                        ...
                                        alert('Failed to create payment Estimate: ' + (response.message ||
                                            'Unknown error'));
                                        utils.updateButton('pay-now-btn', false,
                                            '<i class="fas fa-credit-card"></i> Pay Now');
                                    }
