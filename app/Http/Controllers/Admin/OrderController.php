...
class OrderController extends Controller
{
    ...
    public function updatePaymentStatus(Request $request, Order $order)
    {
        ...
        if (
            $request->payment_status === 'paid' && $oldPaymentStatus !== 'paid' &&
    ...
    public function printInvoice(Order $order)
    {
        try {
            ...
            // Select correct view
            $view = $requestedFormat === 'thermal'
                ? 'admin.orders.print-thermal'
                : 'admin.orders.print-a4';
            $type = $requestedFormat = request()->get('type');

            if ($type == 'print') {
                if ($requestedFormat === 'thermal') {
                    return view(  $view, compact('order', 'companySettings','type'));
                } else {
                    return view(  $view, compact('order', 'companySettings','type'));
                }
            } elseif ($type == 'download') {
                //$view = 'admin.orders.invoice-pdf';
                // Generate PDF from view
                $pdf = Pdf::loadView($view, compact('order', 'companySettings', 'type'));
