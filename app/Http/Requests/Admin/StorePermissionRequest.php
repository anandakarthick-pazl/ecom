...
class StorePermissionRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Auto-generate permission name if module and action are provided
        if ($this->module && $this->action && !$this->name) {
            $this->merge([
                'name' => strtolower($this->module) . '.' . strtolower($this->action)
            ]);
        }

        // Auto-generate display name if not provided
        if ($this->module && $this->action && !$this->display_name) {
            $this->merge([
                'display_name' => ucfirst($this->action) . ' ' . ucfirst($this->module)
            ]);
        }
    }
}
