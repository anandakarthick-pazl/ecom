...
// Temporarily comment out the tenant trait to get basic functionality working
// use App\Traits\BelongsToTenantEnhanced;

class ProductStockNotification extends Model
{
    use HasFactory; // Remove BelongsToTenantEnhanced temporarily
    ...
    // Static methods - Updated to work without tenant trait
    public static function createNotification($productId, $customerData, $sessionId = null)
    {
        ...
        // Get company_id from product, session, or default to 1
        $companyId = $product->company_id ?? session('company_id') ?? 1;
        
        // Check if similar notification already exists (simplified query)
