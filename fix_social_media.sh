...
# Social Media Functionality Fix Script
# Run this script to fix the social media issues
...
if [ -f "social_media_diagnostic.php" ]; then
    php social_media_diagnostic.php
else
    echo "⚠️  Diagnostic script not found. Skipping..."
fi

# Step 5: Create sample social media data (optional)
echo ""
echo "5. Creating sample social media data..."
echo "This will create a test Facebook link for the first company found."

php artisan tinker --execute="
try {
    \$company = \App\Models\SuperAdmin\Company::first();
    if (\$company) {
        \$link = \App\Models\SocialMediaLink::updateOrCreate([
        ...
        echo 'Sample Facebook link created for company: ' . \$company->name . PHP_EOL;
        
        \$link2 = \App\Models\SocialMediaLink::updateOrCreate([
            'company_id' => \$company->id,
            'name' => 'Instagram'
        ], [
        ...
        echo 'Sample Instagram link created for company: ' . \$company->name . PHP_EOL;
        
    } else {
        echo 'No company found. Please create a company first.' . PHP_EOL;
    }
} catch (Exception \$e) {
