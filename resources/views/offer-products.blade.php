...
@section('title', 'Offer Products - ' . ($globalCompany->company_name ?? 'Your Store'))
@section('meta_description', 'Great deals and offers on quality products. Save money with our special discounts and promotional offers.')
...
<div class="container py-4">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-2" id="category-filters">
                        <button class="btn btn-outline-danger btn-sm category-filter {{ request('category', 'all') === 'all' ? 'active' : '' }}" data-category="all">
                            <i class="fas fa-tags me-1"></i>All Offers
                        </button>
                        @foreach($categories as $category)
                        <button class="btn btn-outline-secondary btn-sm category-filter {{ request('category') === $category->slug ? 'active' : '' }}" data-category="{{ $category->slug }}">
                            {{ $category->name }}
                        </button>
                        @endforeach
                    </div>
    ...
    <div id="products-container">
        @include('partials.offer-products-grid', ['products' => $products])
    </div>
    ...
    <div class="container">
        <div class="row text-center">
            <div class="col-md-3">
                ...
                <h5>{{ $enablePagination && method_exists($products, 'total') ? $products->total() : $products->count() }} Offers</h5>
