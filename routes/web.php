...
if (config('app.debug')) {
    require __DIR__ . '/test-social-media.php';
    require __DIR__ . '/debug-pos-categories.php';
    require __DIR__ . '/debug-pagination.php';
}
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'company.context'])->group(function () {
    ...
    Route::prefix('pos')->name('pos.')->group(function () {
        ...
        Route::get('/enhanced-receipt/{sale}', [PosController::class, 'enhancedReceipt'])->name('enhanced-receipt');
        Route::get('/sales/{sale}/download-bill', [PosController::class, 'downloadBill'])->name('download-bill');
        Route::get('/sales/{sale}/download-enhanced-bill', [PosController::class, 'downloadEnhancedBill'])->name('download-enhanced-bill');
        ...
        Route::post('/calculate-offers', [PosController::class, 'calculateOffers'])->name('calculate-offers');
    ...
    Route::prefix('commissions')->name('commissions.')->group(function () {
        ...
        Route::patch('/{commission}/mark-as-paid', [CommissionController::class, 'markAsPaid'])->name('mark-as-paid');
        Route::patch('/{commission}/mark-as-cancelled', [CommissionController::class, 'markAsCancelled'])->name('mark-as-cancelled');
        Route::post('/bulk-mark-as-paid', [CommissionController::class, 'bulkMarkAsPaid'])->name('bulk-mark-as-paid');
        Route::get('/api/dashboard-stats', [CommissionController::class, 'getDashboardStats'])->name('dashboard-stats');
    ...
    Route::prefix('notifications')->name('notifications.')->group(function () {
        ...
        // FIXED: More specific route first, then general route
        // Route::post('/mark-read-by-id/{id}', [NotificationController::class, 'markAsReadById'])->name('mark-read-by-id');
        Route::post('/mark-read-from-body', [NotificationController::class, 'markAsReadByIdFromBody'])->name('mark-read-by-id-body');
...
// Flash Offer Debug Route (for development/testing)
Route::get('/flash-offer-debug', [\App\Http\Controllers\FlashOfferDebugController::class, 'index'])
    ->name('flash-offer.debug');

// Location API Routes (for frontend store locator)
Route::get('/api/locations', [LocationController::class, 'apiLocations'])
    ->name('api.locations');

// ========================================
// TENANT FRONTEND ROUTES (E-COMMERCE)
// ========================================
// These routes are available on tenant domains (e.g., company.local)
...
Route::get('/animation-test', [HomeController::class, 'animationTest'])->name('animation.test');
Route::get('/minimum-order-test', [HomeController::class, 'minimumOrderTest'])->name('minimum.order.test');

// Commission management routes
Route::prefix('admin/commissions')->name('admin.commissions.')->middleware(['auth'])->group(function () {
    Route::get('/', [CommissionController::class, 'index'])->name('index');
    Route::get('/{commission}', [CommissionController::class, 'show'])->name('show');
    Route::get('/{commission}/details', [CommissionController::class, 'getDetails'])->name('details');
    Route::post('/{commission}/mark-paid', [CommissionController::class, 'markAsPaid'])->name('mark-paid');
    Route::post('/{commission}/cancel', [CommissionController::class, 'markAsCancelled'])->name('cancel');
    Route::post('/{commission}/revert-pending', [CommissionController::class, 'revertToPending'])->name('revert-pending');
    Route::post('/bulk-mark-paid', [CommissionController::class, 'bulkMarkAsPaid'])->name('bulk-mark-paid');
});
